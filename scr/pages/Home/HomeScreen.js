...
class MyListItem extends PureComponent {
  render() {
    return (
      <TouchableOpacity
        onPress={() => this.props.goCrudKpi(this.props.item.kpiId)}
      >
...
function HomeScreen(props) {
  ...
  const getCharts = async () => {
    let result = await crudService.get(
      `kpi/${data.userData.signatureId}/${data.userData.personId}`,
      data.token
    );
    ...
    let resultList = await crudService.get(
      `kpi/getListKpis/${data.userData.signatureId}/${data.userData.personId}`,
      data.token
    );
  ...
  renderItem = ({ item }) => <MyListItem item={item} goCrudKpi={goCrudKpi} />;
  ...
  return (
    <>
      ...
      >
        <Text
          h4
          style={{ marginBottom: 10, marginTop: 15, textAlign: "center" }}
        >
          Lista de KPIs
        </Text>
        <View
          ...
          {listCharts.map((chart) => {
            return (
              <>
                ...
                {chart.graphicType == 1 && (
                  <Line data={chart.data} height={chart.height} />
                )}

                {chart.graphicType == 2 && (
                  <Bar data={chart.data} height={chart.height} />
                )}
              </>
            );
          })}
      ...
      {!isLoading && noData && (
        ...
        >
          <TouchableWithoutFeedback onPress={() => goCrudKpi(0)}>
            <Image
              source={require("../../assets/img/graphic.png")}
              style={{ width: 250, height: 220, resizeMode: "cover" }}
            />
          </TouchableWithoutFeedback>
          <TouchableWithoutFeedback onPress={() => goCrudKpi(0)}>
            <Text
            ...
            >
              Clique aqui e adicione o seu primeiro KPI.
            </Text>
          </TouchableWithoutFeedback>
        </View>
      )}
